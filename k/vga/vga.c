...
#include "serial.h"
#include "syscalls/syscalls_io.h"
#include <k/kstd.h>
...
struct image *load_image(const char *path) {
  static struct bitmap_header bmp;
  ...
  img.width = abs(bmp.width);
  img.height = abs(bmp.height);

  printf("img.width %d\n", img.width);
  printf("img.height %d\n", img.height);

  if (img.width > MAX_WIDTH || img.height > MAX_HEIGHT) {
    printf("Image dimensions exceed the maximum allowed size\n");
    goto err_img;
  }

  int ppl = (bmp.size - (img.width * img.height)) / img.height;

  off_t seeked_offset = seek(fd, bmp.offset, SEEK_SET);
  if (seeked_offset == (off_t)-1) {
    printf("seek error\n");
    goto err_img;
  }
  ...
  for (unsigned int i = 0; i < img.height; i++) {
    ...
    // print_buffer_hexa(img.data[i], img.width);

    if (rc < (int)img.width)
      goto err_img;
    if (seek(fd, ppl, SEEK_CUR) == (off_t)-1) {
      printf("seek error\n");
      goto err_img;
    }
  }
...
void clear_image(struct image *image) {
  for (unsigned int i = 0; i < image->height; i++)
    memory_release(image->data[i]);
  memory_release(image->data);
  memory_release(image);
}
...
struct anim *load_anim(char *paths, int delay) {
  ...
  if (!(anim = memory_reserve(sizeof(struct anim))))
    return NULL;
  ...
  if (!(anim->imgs = memory_reserve(anim->nr_img * sizeof(struct image *)))) {
    memory_release(anim);
    return NULL;
  }
  memset(anim->imgs, 0, anim->nr_img * sizeof(struct image *));

  if (!(p = strdup(paths))) {
    memory_release(anim->imgs);
    memory_release(anim);
    return NULL;
  }
  bck = p;
