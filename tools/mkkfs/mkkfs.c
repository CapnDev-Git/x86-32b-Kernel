...
#define KFS_MAX_FILE_SZ                                                        \
  ((KFS_DIRECT_BLK + KFS_INDIRECT_BLK * KFS_INDIRECT_BLK_CNT) * KFS_BLK_DATA_SZ)
 ...
 */
static void kfs_write_superblock(int romfd, const char *fsname, u32 blk_cnt,
                                 u32 files_cnt) {
  struct kfs_superblock sblock = {.magic = KFS_MAGIC,
#ifdef DEBUG
                                  .ctime = 0,
#else
                                  .ctime = time(NULL),
  ...
  sblock.cksum = kfs_checksum(&sblock, sizeof(sblock) - sizeof(sblock.cksum));
 ...
 */
static u32 kfs_write_inode(int romfd, int fd, struct kfs_inode *inode,
                           u32 blk_idx) {
  ...
  for (size_t i = 0; i < KFS_INDIRECT_BLK; ++i) {
    ...
    iblock_idx.cksum = kfs_checksum(&iblock_idx, sizeof(iblock_idx) -
                                                     sizeof(iblock_idx.cksum));
 ...
 */
static u32 kfs_write_files(int romfd, char **files, size_t nb_files,
                           size_t blkoff) {
  ...
  for (size_t i = 0; i < nb_files; ++i, inode_off++) {
    ...
    if (st.st_size > KFS_MAX_FILE_SZ)
      err(1, "file \"%s\" of size %zu is too large to fit in kfs", files[i],
          st.st_size);
